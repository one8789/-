...
import { X, Copy, CheckCircle, Tag, AlertCircle, Sparkles, Send, Calculator, Trash2, ChevronDown, ShieldAlert, Lock, Unlock, Video, MessageCircle, ShoppingBag, ExternalLink, ArrowRight, Store, HeartHandshake, Clock, QrCode, Palette, Shield } from 'lucide-react';
...
const CheckoutModal: React.FC = () => {
  ...
  const getOrderText = () => {
    ...
    if (consultationMode) { /* ... consultation text generation ... */ } 
    else {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end md:items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in" onClick={() => toggleModal(false)}>
      ...
      <div className={`bg-white w-full rounded-t-3xl md:rounded-3xl shadow-2xl overflow-hidden animate-slide-up flex flex-col max-h-[90vh] md:max-h-[85vh] transition-all duration-300 ${consultationMode ? 'md:max-w-4xl' : 'md:max-w-xl'}`} onClick={(e) => e.stopPropagation()}>
